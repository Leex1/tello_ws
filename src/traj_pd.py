...
class DroneControllerNode:
    def __init__(self):
        ...
        self.desired_angular_speed = 0.5  # Desired angular velocity of drone
    ...
    # def traj_callback(self):
        ...
        # 追踪a点轨迹
        if(self.traj_point):
            self.desired_pose_.pose.position.x = 0.0
            self.desired_pose_.pose.position.y = 0.0
            self.desired_pose_.pose.position.z = 1.5
            self.desired_pose_.pose.orientation = current_pose.orientation
        # 追踪圆轨迹
        if(abs(self.last_error_linear_x)<0.3 and abs(self.last_error_linear_y)<0.3):
            self.traj_point = False
            self.desired_pose_.pose.position.x = self.desired_radius * math.cos(self.desired_angular_speed * rospy.get_time())
            self.desired_pose_.pose.position.y = self.desired_radius * math.sin(self.desired_angular_speed * rospy.get_time())
